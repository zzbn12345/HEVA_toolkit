...
The default environment installs only Pydantic. It covers the canonical record contract,
...
The extraction profile adds PyMuPDF, spaCy, and python-docx. These modules turn PDF or
Word evidence into candidate records. They must not decide rights, approve mappings,
...
The optional application lives under `app/` and has its own environment. FastAPI, Uvicorn,
