...
The entry point (`src/extract_highlights.py`) scans the `data/` directory and dynamically routes files depending on their extension:
* **PDF Documents**: Routed to `PyMuPDF` (`fitz`) processing in `src/pdf_extractor.py`.
* **Word Documents**: Routed to `python-docx` processing in `src/docx_extractor.py`.
