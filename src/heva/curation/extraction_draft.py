...
from heva.curation.color_configuration_registry import selected_color_configuration
from heva.curation.contract import CURRENT_SCHEMA_VERSION, HEX_COLOR, validate_record
...
def promote_extraction_draft(project_root: str | Path, document_id: str):
    ...
    root = Path(project_root).resolve()
    configuration = selected_color_configuration(root)
    from heva.curation.color_configuration_registry import apply_selected_configuration_to_document
    from heva.curation.extraction_session import persist_extraction_results
